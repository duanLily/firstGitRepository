...
class PropertyContainer {
	...
	/*
	 * Holds the AccessType indicated for use at the class/container-level for
	...
	PropertyContainer(XClass clazz, XClass entityAtStake, AccessType defaultClassLevelAccessType) {
		...
		if (defaultClassLevelAccessType == AccessType.DEFAULT) {
			// this is effectively what the old code did when AccessType.DEFAULT
			// was passed in
			// to getProperties(AccessType) from AnnotationBinder and
			// InheritanceState
	...
	private void collectPersistentAttributesUsingLocalAccessType(
		...
		while (propertyIterator.hasNext()) {
			...
			// HHH-10242 detect registration of the same property getter twice -
			// eg boolean isId() + UUID getId()
	...
	private void collectPersistentAttributesUsingClassLevelAccessType(
		...
		if (classLevelAccessType == AccessType.FIELD) {
			...
			for (XProperty getter : getters) {
				...
				// HHH-10242 detect registration of the same property getter
				// twice - eg boolean isId() + UUID getId()
				final XProperty previous = persistentAttributesFromGetters.get(name);
				if (previous != null) {
					throw new org.hibernate.boot.MappingException(
	...
	// // use field access for this property even if the default access type for
	// the class is AccessType.PROPERTY
	// AccessType accessType = AccessType.getAccessStrategy( access.value() );
	// if (accessType == AccessType.FIELD) {
	// propertyAccessMap.put(property.getName(), property);
	// }
	// else {
	// LOG.debug( "Placing @Access(AccessType.FIELD) on a field does not have
	// any effect." );
	// }
	// }
	//
	...
	// }
	// else {
	// LOG.debug( "Placing @Access(AccessType.PROPERTY) on a field does not have
	// any effect." );
	// }
	// }
	// }
	...
	// /**
	// * Retrieves all properties from the {@code xClass} with the specified
	// access type. This method does not take
	// * any jpa access rules/annotations into account yet.
	// *
	// * @param access The access type - {@code AccessType.FIELD} or {@code
	// AccessType.Property}
	// *
	// * @return A maps of the properties with the given access type keyed
	// against their property name
	// */
	// private TreeMap<String, XProperty> initProperties(AccessType access) {
	// if ( !( AccessType.PROPERTY.equals( access ) || AccessType.FIELD.equals(
	// access ) ) ) {
	// throw new IllegalArgumentException( "Access type has to be
	// AccessType.FIELD or AccessType.Property" );
	// }
	//
	...
	// XProperty>();
	// List<XProperty> properties = xClass.getDeclaredProperties(
	// access.getType() );
	// for ( XProperty property : properties ) {
	// if ( mustBeSkipped( property ) ) {
	// continue;
	// }
	// // HHH-10242 detect registration of the same property twice eg boolean
	// isId() + UUID getId()
